...
interface Rectangle {
    x: number;           // X-Location of Rectangle
    y: number;           // Y-Location of Rectangle
    w: number;           // Width of Rectangle
    h: number;           // Height of Rectangle

    primaryClr: RGBA;    // Backrgound/primary Color Object
    secondaryClr?: RGBA; // Secondary Color Object (Optional)
}

/** Button Data Interface */
interface Button {
    rectShape: Rectangle;       // Shape of Button (Rectangle Object)

    text?: string;              // Holds String (Optional)
    textPos?: Position2D;       // Text Position

    fn?: Function;              // Holds Function (Optional)
    fnArgs?: number;            // Number of Arguments (Optional)
}
...
interface Position2D {
    x: number;          // X-Position Value
    y: number;          // Y-Position Value
}

/** General 2D Dimensions (Shape) */
interface Shape2D {
...
class Menu {
    // Menu Data
    private mainCoorSys: CoordinateSystem;
    private ctx: CanvasRenderingContext2D;
    private isActive: Boolean;

    // Button Data
    private menuBtn: Rectangle;
    private menuBox: Rectangle;
    private closeMenuBtn: Rectangle;
     ...
     */
    constructor(coorSys: CoordinateSystem, ctx: CanvasRenderingContext2D) {
        // Initiate Menu Data
    ...
    private initMenuButtons(): void {
        ...
        // Text Data
        const txtOffset: Position2D = { x: 10, y: 8 };
     ...
     */
    public clickedButton(index: number | Position2D): void {
        // Value Was Given
        if (!isNaN(index as number)) {
            // Make sure Index is found in buttons array
            if (index < this.buttons.length && index >= 0) {
                this.buttons[index as number].fn();
            }
        ...
        // Check for Mouse Position within any of the Buttons
            ...
            for (const btn of this.buttons) {
                ...
                // If mouse within
                if (Menu.within(obj, shape, (index as Position2D))) {
     ...
     */
    public setMenuBtnColor(plate?: RGBA, burgerSlide?: RGBA): void {
     ...
     */
    public getButtonPlateColor(): RGBA {
     ...
     */
    public getButtonBurgerSlidesColor(): RGBA {
     ...
     */
    public getButtonPosition(): Position2D {
    ...
    */
     ...
     */
    public getCloseMenuBtnPosition(): Position2D {
     ...
     * @returns Shape2D Object for Button Shape
     */
    public getCloseMenuBtnShape(): Shape2D {
    ...
    private drawMenu(): void {
        ...
        for (let i = 0; i < this.buttons.length; i++) {
            ...
            this.ctx.fillText(
                this.buttons[i].text,
                this.buttons[i].textPos.x,
                this.buttons[i].textPos.y
            );
        }
     ...
     */
    private colorToString(rgba: RGBA): string {
     ...
     */
    static within(pos: Position2D, objShape: Shape2D, mousePos: Position2D): Boolean {
        if ((pos.x) < mousePos.x &&
            (pos.y) < mousePos.y &&
